...
MEDIA_FOLDER = 'media'  # Folder to save media and metadata
...
def save_to_db(status, confidence, media_format):
    ...
    platform = "instagram"  # Set as default
...
def pred(image_path: str):
    ...
    face = mtcnn(img)
    if face is None:
        return "no_face_detected", 0  # Return a specific message
...
# Function to generate the SHA-256 hash of the file
...
# Function to save the uploaded media and metadata
def save_media_and_metadata(file, owner, media_format):
    try:
        ...
        file.save(file_path)  # Save the media file
...
def upload_file():
    owner = "DeepTracers"  # Set default owner for metadata
    ...
    if 'image' in request.files:
        ...
        try:
            ...
            # Ensure reverse search results are returned regardless of prediction
            reverse_search_results = []
            if status == "real":
                # Directly use the upload path for reverse search
                reverse_search_results = reverse_image_search(upload_path)
            else:
                logging.info(f"Deepfake detected: Skipping saving of {file.filename}")

            # Save metadata even if the prediction is fake
            saved_file_path, metadata_file_path = save_media_and_metadata(file, owner, media_format)

            return jsonify({
                'prediction': status, 
                'confidence': confidence,
                'reverse_search_results': reverse_search_results,  # Always return this
                'metadata_file': metadata_file_path,  # Include metadata file path
